...
extern double Phi;
extern double R;
extern double horiTranslate;
extern double vertTranslate;
...
extern pair<point, bool> inclinedPlaneNormal;

extern point dragForceDirection;
extern double dragForceMagnitude;

// used for switching dragging direction in interactive mode
extern int dragCase;
...
// negates a point/vector
// struct point source, dest
#define pNEG(source, dest)\
\
  (dest).x = -(source).x;\
  (dest).y = -(source).y;\
  (dest).z = -(source).z;
