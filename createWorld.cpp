  ...
  for (i=0; i<= jello.resolution-1; i++)
    for (j=0; j<= jello.resolution-1; j++)
      for (k=0; k<= jello.resolution-1; k++)
      {
        ...
        // set the force at curr point
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].x = -sin(x) / (x*x+y*y+z*z)/10; 
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].y = logb(y) / (x*x+y*y+z*z)/10;
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].z = 0;
      }

  // way to access:
  // cout<<jello.forceField[1].x<<endl;
    ...
    for (j=0; j<=7; j++)
	    for (k=0; k<=7; k++)
      {
        jello.p[i][j][k].x=1.0 * i / 7 ;
	  	  jello.p[i][j][k].y=1.0 * j / 7 +1;
		    jello.p[i][j][k].z=1.0 * k / 7 ;
        // can set some special points
        // if ((i==7) && (j==7) && (k==7))
        // {
        ...
        // if ((i==0) && (j==0) && (k==0))
        // {
        //   jello.p[i][j][k].x=0.0 - 1.0 / 7;
	    	//   jello.p[i][j][k].y=0.0 - 1.0 / 7;
		    //   jello.p[i][j][k].z=0.0 - 1.0 / 7;
        // }
      }
