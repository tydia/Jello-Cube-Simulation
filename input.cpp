...
#define DRAG_DIAG1 0
#define DRAG_DIAG2 1
#define DRAG_LEFT_N_RIGHT 2
#define DRAG_FRONT_N_BACK 3
...
void saveScreenshot(int windowWidth, int windowHeight, char *filename)
{
  ...
  Pic * in = pic_alloc(windowWidth*2, windowHeight*2, 3, NULL);

  printf("File to save to: %s\n", filename);

  for (int i=windowHeight*2-1; i>=0; i--) 
  {
    glReadPixels(0, windowHeight*2-i-1, windowWidth*2, 1, GL_RGB, GL_UNSIGNED_BYTE,
...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iLeftMouseButton) {
      ...
      dragForceDirection.x = 0;
      dragForceDirection.y = 0;
      dragForceDirection.z = 0;


    if (dragCase == DRAG_DIAG1) {
      dragForceDirection.x = vMouseDelta[0];
      dragForceDirection.y = vMouseDelta[0];
      dragForceDirection.z = -vMouseDelta[1];
    }

    if (dragCase == DRAG_DIAG2) {
      dragForceDirection.x = -vMouseDelta[0];
      dragForceDirection.y = vMouseDelta[0];
      dragForceDirection.z = -vMouseDelta[1];
    }

    if (dragCase == DRAG_LEFT_N_RIGHT) {
      dragForceDirection.x = 0;
      dragForceDirection.y = vMouseDelta[0];
      dragForceDirection.z = -vMouseDelta[1];
    }

    if (dragCase == DRAG_FRONT_N_BACK) {
      dragForceDirection.x = -vMouseDelta[0];
      dragForceDirection.y = 0;
      dragForceDirection.z = -vMouseDelta[1];
    }
    if (dragCase!=-1) 
      dragForceMagnitude = ( sqrt(vMouseDelta[0]*vMouseDelta[0]+
                                vMouseDelta[1]*vMouseDelta[1])/20);
    // std::cout<<dragForceMagnitude<<endl;
  }
...
void arrowKeyFunc (int key, int x, int y) {
  switch (key)
  {
    case GLUT_KEY_LEFT:
      horiTranslate-=cos(0.01);
      break;
    case GLUT_KEY_RIGHT:
      horiTranslate+=cos(0.01);
      break;
...
void keyboardFunc (unsigned char key, int x, int y)
{
  switch (key)
  {
    ...
    case 'i':
      dragCase++;
      if(dragCase == 4) dragCase = 0;
      printf("Dragging up and down moves the cube up or down.\n");
      printf("Dragging left and right moves the cube on x-y plane.\n");
      printf("For dragging direction, you have different options.\n");
      if (dragCase == 0) {
        printf("CURRENT Mouse Drag Mode: Diagonal Drag of (-x,y,z) to (x,-y,z)\n");
      }
      ...
      if (dragCase == 2) {
        printf("CURRENT Mouse Drag Mode: Drag between Left and Right Walls\n");
      }

      if (dragCase == 3) {
        printf("CURRENT Mouse Drag Mode: Drag between Front and Back Walls\n");
      }
      printf("Press 'i' again to switch dragging direction in x-y plane.\n\n");
      break;
