...
void showInclinedPlane(struct world * jello) {
  ...
  point random_vec;
  pMAKE(1,2,3, random_vec);
  // calculate two vectors that are perpendicular to normal
  // the method i use is the same as finding a local coordinate
  // system
  point v1,v2;
  CROSSPRODUCTp(n, random_vec, v1);
  CROSSPRODUCTp(n, v1, v2);

  // sample a point on plane.
  // set x=y=0, then solution to z is cz+d=0->z=-d/c
  pMAKE(0,0,-d/c, p0);
